...
def assign_tags_to_stock(
    ...
    """给股票打标签（替换式：传入的tag_ids会替换该股票的所有标签）"""
    stock = session.exec(select(Stock).where(Stock.symbol == payload.symbol)).first()
    if not stock:
        raise HTTPException(status_code=404, detail="股票不存在")
